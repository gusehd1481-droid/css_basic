...
## 글꼴 사용 시 주의사항
* 모든 컴퓨터 기본 글꼴 (고딕, 궁서, 바탕 등)이 아니라면 반드시 웹글꼴을 사용하여 웹접근성을 높여줘야함.
* 웹글꼴 연결 순서는 작성한 css보다 반드시 위에 작성하기!
* 글꼴명이 공백이 포함된 경우 따옴표(큰, 작은 따옴표 상관x) 붙여서 작성하기!
## 자주 사용하는  CSS 속성 모음
...
* `table` -> `tfoot` -> `tr` -> `td`
## 수평/수직 열 합치기
* 합치기 속성은 무조건 `th` 또는 `td`로 태그에 입력 가능!
* 수평 열 합치기
...
* `<input autofocus placeholder="">`
    * acutofocus : 페이지 접속 시 바로 커서 위치 활성화
...
* `<button type="button"이벤트="자바스크립트명령어작성">
    ...
    * `button:hover {}` : 버튼에 마우스올렸을 시 디자인
# 웹글꼴 `<link>`, `@font-face`
## `<link>` 사용법과 특징 (index안 웹글꼴 설정)
* `head` 태그 안 `reset.css` 연결보다 위에 작성
* `@font-face`에 비해 사용이 간편함.
* 작성한 html 에서만 사용할 수 있다는 단점있음.
## `@font-face` 사용법과 특징
* `reset.css` 파일 내 가장 위쪽 라인에 작성
* `@font-face {`
    * `font-family:'사용할 글꼴 이름 임의작성'`        <!-- 필수 -->
    * `src:url(글꼴주소)`                            <!-- 필수 -->
    * `font-weight:글꼴굵기(200~700 글꼴에 따라 다름)` <!-- 선택 -->
    * `font-style:기울기(normal, italic 등)`         <!-- 선택 -->   
    * `font-display:swap;`                          <!-- 선택 -->
    * `}`
* reset에 한번 연결해두면 모든 html 에서 사용 가능
* `@font-face {font-family:'사용할글꼴명'}`   (예) 컴퓨터 글꼴설치
* `선택자 {font-family:'웹글꼴로 불러온 글꼴명'}`  (예) 포토샵 글꼴사용
## CSS 레이아웃 정렬 속성
### display
...
* `float:right;` : 형제 요소들을 오른쪽으로 정렬
    * 2개 이상 작성 시 역순으로 정렬됨.
* `float:none;` : float 제거
* `clear:both;` : 이전 형제에서 작성된 float 정렬 해제
### position
* `position:relative`
...
* `li {width:calc((100% - (10px * 3)) / 4);}`
    * 4개의 li에 각 10px씩 사이여백을 주기 위해 전체 부모 100% 너비 중 10*3 총 30px을 뺴고 나머지 값을 4로 나누기
* `a {display:block; height:calc(100%-50px);}`
    * a의 크기를 인식하게 만들고 50px을 뺀 나머지 부모크기주기
## 수평/수직 정렬 레이아웃 속성 Flex
### 정렬 순서
1. 정렬하고자 하는 **두개 이상 형제관계** 형제 대상 체크
    * `ul > li*5 > a`  -> 형제 `li`
...
* `flex-flow:column nowrap` : 메인축 수직, 줄바꿈 안함
...
* `justify-content:`
    ...
    * `space-arround` : 메인축이 수직이면 위-아래 양쪽여백 주고 균등배치 , 수평이면 왼쪽-오른쪽 양쪽여백 주고 균등배치
...
* `:hover` 선택자로 인해 기존속성에서 다른속성으로 변경되는 CSS에 애니메이션을 주고 싶을 때
    ...
    * 2초 기다리고 1초동안 일정속도로 color 속성에 애니메이션 적용
## animation + @keyframes
### 적용 순서
1. `@keyframes 애니메이션이름 {적용분기}`
2. `선택자 {animation:키프레임애니이션이름 추가속성}`
### 개념 및 주의사항
...
* `선택자 {animation:a 1s ease 0.5s alternate 3;}`
    * 특정 선택자에 0.5초 기다리고 1초동안 a 애니메이션을 ease 가속도로 정방향->역방향 순서로 3번 재생하기
## transform
...
* `transform:roate(20deg);`= 시계방향. 반시계 방향은 `-20deg`
